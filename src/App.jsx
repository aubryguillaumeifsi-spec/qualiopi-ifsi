...
export default function App() {
      ...
      if (snap.exists() && snap.data().campaigns) {
        ...
        if (oldSnap.exists() && oldSnap.data().liste) {
          finalCamps = [{
            ...
            auditDate: oldSnap.data().auditDate || "2026-10-15",
  ...
  async function saveData(newCampaigns) {
    ...
    setCampaigns(newCampaigns);
    try {
      const etabRef = doc(db, "etablissements", userProfile.etablissementId);
      await setDoc(etabRef, { campaigns: newCampaigns }, { merge: true });
    } catch (e) { console.error(e); }
  }
