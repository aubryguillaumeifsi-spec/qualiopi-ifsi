...
function MainApp() {
  ...
  const handleChangePassword = async (e, isForced) => {
  ...
  const saveModal = (updated, action) => {
    try {
      ...
      setModalCritere(null); // On ferme toujours pour ne pas bloquer l'utilisateur
    }
  };
  ...
  const exportToExcel = async () => {
    alert("Veuillez patienter pendant le téléchargement...");
  };
      ...
      <div className="no-print" style={{ background: "white", borderBottom: "1px solid #e2e8f0", padding: "14px 32px", boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
        <div style={{ maxWidth: "1440px", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            <button onClick={() => signOut(auth)} style={{ color: "#ef4444", border: "1px solid #fca5a5", padding: "6px 12px", borderRadius: "6px", background: "#fef2f2", cursor: "pointer", fontWeight: "bold" }}>Déconnexion</button>
          </div>
        </div>
      </div>
