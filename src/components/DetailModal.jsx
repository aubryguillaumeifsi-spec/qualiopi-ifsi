            ...
            return (
              <label key={r} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "9px 14px", cursor: "pointer", background: checked ? "#eff6ff" : "white", borderBottom: "1px solid #f9fafb" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "11px", color: cCfg.text, fontWeight: "600" }}>{role}</div>
                </div>
              </label>
            );
          })}
      ...
      {selected.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginTop: "6px" }}>
          {selected.map(r => {
            ...
            const cCfg = ROLE_COLORS[role] || ROLE_COLORS["Défaut"];
            return (
              <span key={r} style={{ background: disabled ? "#f3f4f6" : cCfg.bg, color: disabled ? "#6b7280" : cCfg.text, border: `1px solid ${disabled ? "#d1d5db" : cCfg.border}`, borderRadius: "20px", padding: "3px 10px", fontSize: "11px", display: "flex", alignItems: "center", gap: "6px", fontWeight: "600" }}>
                {r.split("(")[0].trim()}
                {!disabled && <button onClick={() => toggle(r)} style={{ background: "none", border: "none", cursor: "pointer", color: cCfg.text, opacity: 0.7, fontSize: "14px", lineHeight: 1, padding: 0 }}>x</button>}
              </span>
            );
          })}
        </div>
      ...
      <div className="modal-content" style={{ background: "white", borderRadius: "16px", padding: "32px", width: "100%", maxWidth: "1000px", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 60px rgba(0,0,0,0.15)" }} onClick={e => e.stopPropagation()}>
        ...
        <div className="print-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "32px", marginBottom: "24px" }}>
          ...
          <div className="print-col">
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px", borderBottom: "2px solid #e2e8f0", paddingBottom: "8px" }}><span style={{ fontSize: "18px" }}>✍️</span><h3 style={{ fontSize: "14px", fontWeight: "800", color: "#1e3a5f", margin: 0, textTransform: "uppercase", letterSpacing: "0.5px" }}>Notre Réponse IFSI</h3></div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "16px" }}>
              <div>
                <label style={lbl}>Statut actuel</label>
                <select className="no-print" disabled={isReadOnly || isAuditMode} value={data.statut} onChange={e => setData({ ...data, statut: e.target.value })} style={{ ...inp, background: STATUT_CONFIG[data.statut].bg, color: STATUT_CONFIG[data.statut].color, fontWeight: "600", border: `1.5px solid ${STATUT_CONFIG[data.statut].border}` }}>{Object.entries(STATUT_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select>
            ...
            <div style={{ marginBottom: "16px" }}><label style={lbl}>Responsable(s) assigné(s)</label><MultiSelect disabled={isReadOnly || isAuditMode} selected={data.responsables} onChange={val => setData({ ...data, responsables: val })} /><div className="print-value" style={{ display: "none" }}>{data.responsables.length > 0 ? data.responsables.map(r => r.split("(")[0].trim()).join(", ") : "Aucun responsable assigné"}</div></div>
            <div style={{ marginBottom: "16px" }}><label style={lbl}>{isAuditMode ? "Preuves (Liens, Emplacements)" : "✅ Preuves FINALISÉES (Liens, Emplacements)"}</label><textarea className="no-print" readOnly={isReadOnly || isAuditMode} value={data.preuves || ""} onChange={e => setData({ ...data, preuves: e.target.value })} rows={5} placeholder="Ex: Livret d'accueil p.12, Lien Sharepoint..." style={{ ...inp, width: "100%", boxSizing: "border-box", resize: "vertical", lineHeight: "1.6", borderColor: (isReadOnly || isAuditMode) ? "#d1d5db" : "#6ee7b7", background: (isReadOnly || isAuditMode) ? "#f9fafb" : "#f0fdf4" }} /><div className="print-value" style={{ display: "none", whiteSpace: "pre-wrap", background: "#f0fdf4", padding: "10px", borderLeft: "3px solid #10b981", borderRadius: "4px" }}>{data.preuves || "—"}</div></div>
            {!isAuditMode && (
              <>
                <div style={{ marginBottom: "16px" }}><label style={lbl}>⏳ Preuves EN COURS d'élaboration</label><textarea className="no-print" readOnly={isReadOnly} value={data.preuves_encours || ""} onChange={e => setData({ ...data, preuves_encours: e.target.value })} rows={3} placeholder="Ex: Trame d'entretien en cours de rédaction, en attente signature..." style={{ ...inp, width: "100%", boxSizing: "border-box", resize: "vertical", lineHeight: "1.6", borderColor: isReadOnly ? "#d1d5db" : "#fcd34d", background: isReadOnly ? "#f9fafb" : "#fffbeb" }} /><div className="print-value" style={{ display: "none", whiteSpace: "pre-wrap", background: "#fffbeb", padding: "10px", borderLeft: "3px solid #f59e0b", borderRadius: "4px" }}>{data.preuves_encours || "—"}</div></div>
                <div style={{ marginBottom: "16px" }}><label style={lbl}>Commentaires / Attendus demandés par l'évaluateur</label><textarea className="no-print" readOnly={isReadOnly} value={data.attendus || ""} onChange={e => setData({ ...data, attendus: e.target.value })} rows={3} placeholder="Ex: L'auditeur a demandé à ce qu'on précise la date sur la feuille d'émargement..." style={{ ...inp, width: "100%", boxSizing: "border-box", resize: "vertical", lineHeight: "1.6", background: isReadOnly ? "#f9fafb" : "#f8fafc", borderColor: "#e2e8f0" }} /><div className="print-value" style={{ display: "none", whiteSpace: "pre-wrap" }}>{data.attendus || "—"}</div></div>
                <div style={{ marginBottom: "28px" }}><label style={lbl}>Notes internes IFSI (Points de vigilance)</label><textarea className="no-print" readOnly={isReadOnly} value={data.notes || ""} onChange={e => setData({ ...data, notes: e.target.value })} rows={2} placeholder="Ex: Attention à bien valider ce document en Copil..." style={{ ...inp, width: "100%", boxSizing: "border-box", resize: "vertical", lineHeight: "1.6" }} /><div className="print-value" style={{ display: "none", whiteSpace: "pre-wrap", fontStyle: "italic", color: "#6b7280" }}>{data.notes || "—"}</div></div>
              </>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid #e2e8f0", paddingTop: "20px" }}>
          <button className="no-print" onClick={() => window.print()} style={{ padding: "8px 16px", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "8px", color: "#1d4ed8", cursor: "pointer", fontSize: "12px", fontWeight: "600", display: "flex", alignItems: "center", gap: "6px" }}><span>📄</span> Imprimer cette fiche</button>
